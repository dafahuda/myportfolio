...
export function CertificateCard({
  ...
  return (
    <div
      {...rest}
      className="group relative w-full overflow-visible rounded-2xl bg-zinc-800 shadow-lg transition-all duration-300 hover:shadow-2xl flex flex-col"
    >
      ...
      {/* Content Section */}
      <div className="relative space-y-4 p-6 transition-all duration-300 group-hover:pt-8 flex-grow flex flex-col">
        <h3 className="text-2xl font-bold text-white transition-colors duration-300 group-hover:text-violet-400">
          {title}
        </h3>

        <p className="text-gray-300 leading-relaxed h-20">{description}</p>
        <p className="text-gray-400 text-sm font-semibold mt-auto">Issuer: {issuer}</p>

        <button
          onClick={onButtonClick}
          className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-violet-700 px-4 text-sm font-medium text-slate-50 shadow transition-colors hover:bg-violet-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 group-hover:gap-4 mt-auto"
        >
          Lihat Sertifikat
