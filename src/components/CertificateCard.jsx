 ...
 * }} props
 */
export function CertificateCard({ certificates, title, description, issuer, onButtonClick, maxVisible = 4, ...rest }) {
  const visibleCerts = certificates.slice(0, maxVisible);

  return (
    <div {...rest} className="group relative w-full max-w-sm overflow-visible rounded-2xl bg-zinc-800 shadow-lg transition-all duration-300 hover:shadow-2xl">
      {/* Image Container with Stacking Effect */}
      <div className="relative h-56 overflow-visible pt-16">
      ...
      {/* Content Section */}
      <div className="relative space-y-4 p-6 transition-all duration-300 group-hover:pt-8">
        ...
        <button
          ...
          className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-violet-700 px-4 text-sm font-medium text-slate-500 shadow transition-colors hover:bg-violet-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 group-hover:gap-4"
        >
          Lihat Sertifikat
